...
`sissy-serverd` links the Swift packages below, so their code ships inside
Sissy.app. Their terms are reproduced here. Everything Sissy itself is made of
is covered by [LICENSE](LICENSE).

## SwiftNIO

<https://github.com/apple/swift-nio>, Apache License 2.0. Its NOTICE, verbatim:
...
<https://github.com/apple/swift-system>: Apache License 2.0 with the Runtime
Library Exception. SwiftNIO pulls them in. The exception waives the attribution
sections 4(a), 4(b) and 4(d) would otherwise ask for when a compiler embeds
them in a binary; it is reproduced below, and so is the credit.

## Credits

No code from these ships in Sissy. It reads the numbers they publish.

- [`ccusage`](https://github.com/ryoppippi/ccusage): the cost oracle Sissy
  measures itself against, and where its reading of the Claude Code and Codex
  JSONL schemas comes from.
- [LiteLLM](https://github.com/BerriAI/litellm): the model price table the
  daemon fetches at runtime, which is what `ccusage` prices from too.
