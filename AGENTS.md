...
- **The daemon owns what has to survive the app quitting; the app owns the rest.** `sissy-serverd` is a LaunchAgent and keeps counting after Sissy quits, so anything that must keep working with no menu bar icon on screen belongs there and gets a control surface in the app — `KeepAwake` is the worked example, since the app can be quit under a live hold. Anything that means nothing while nobody is at the machine belongs in the app; a notification is the counter-example, because there is no one to read it.
