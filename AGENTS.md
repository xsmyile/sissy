...
- **Pricing tables are hardcoded but overridable.** `Pricing.swift` (Anthropic) and `OpenAIPricing.swift` (OpenAI) reflect public prices at writing time; override via `server.json.pricingOverride` if rates change. A weekly `pricing-drift` CI job (`scripts/check_pricing_drift.py`) re-checks both tables against LiteLLM and fails when a new or repriced model would misbill.
