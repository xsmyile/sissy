...
- **ccusage is the cost oracle.** It prices from LiteLLM too, so reading LiteLLM directly is what keeps Sissy agreeing with the number users cross-check. Where the two would disagree, match ccusage — subscription users never see a token invoice, so agreeing with the community tool beats agreeing with a hypothetical bill. The `pricing-oracle` workflow asserts exact agreement on a synthetic fixture. Verify pricing questions by **measuring** against ccusage, not by reading pricing pages: two independent readings of the long-context rules (an earlier pass here and a Codex review) both got them wrong, and a three-point measurement settled it in minutes.
- **Known residual vs ccusage: ~0.06% on real logs.** Sub-agent (`isSidechain: true`) entries are written more than once within a session file and the two tools collapse a handful of them differently — localized to the sub-agent models, with the main model matching exactly. Not worth chasing; the fixture-based oracle is the regression guard.
