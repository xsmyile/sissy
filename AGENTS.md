...
- **Nothing Sissy does to the machine outlives Sissy.** The daemon keeps *counting* after the app quits, because the numbers are read from files that are there either way. It must not keep *doing* anything: `KeepAwake` is the worked example, and the rule it earned — the mode persists, the power assertion is held only while a client is connected. A hold nobody can see, on a Mac whose menu bar has no Sissy in it, is a battery complaint with no path back to its cause. This is also the rule the [single-process migration](docs/single-process-migration.md) is settling for good, by removing the split that makes the distinction necessary.
