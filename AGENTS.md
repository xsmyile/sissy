...
**Two login items, one per half.** `Models/LoginItemController.swift` registers the app itself through `SMAppService.mainApp`; the daemon's agent is registered separately and carries `RunAtLoad`, so once the Server is on it starts at login whether or not the app does. The app's login item therefore only decides whether the menu bar icon comes back. Neither state is mirrored in `preferences.json`: `SMAppService` is the record, and a user who removes Sissy from System Settings' Login Items would leave a mirrored flag claiming something the system had already undone.

## Conventions specific to this repo

- **One mascot, two poses.** `SissyModel.mascotAssetName` is the resting silhouette; `mascotSleepingAssetName` is the same silhouette with its eye shut, and it is what the menu bar *and* the panel header rest on while nothing is reaching the app. Both are template images, so surfaces tint them and never ship a second copy. The icon is drawn at full opacity in both states — the shut eye replaced a 40% dim, so don't reintroduce one. Sizing is per-surface: the menu bar draws it at 17 pt because the asset carries 20 pt of ink where an unconfigured SF Symbol measures 15, and at native size it reads a third taller than its neighbours. Copy the image before resizing — `NSImage(named:)` returns the catalogue's shared instance.
- **The mascot's motion is one 24-frame sequence.** `Assets.xcassets/MascotMotion` holds a blink rendered at 60 fps as vector imagesets, and `SissyMenuBarMotion` slices it into the whole blink plus its two halves. Frames 6–9 are the byte-identical shut-eye hold, which is why the closing half stops at 6 and the opening half starts at 9. `SissyMenuBarAnimator` owns `button.image` from the moment it exists — anything else assigning it drops the remaining frames of a gesture. A blink fires when a frame lands (`SissyModel.lastFrameAt`, rate-limited by `dataBlinkCooldown`, because the readers coalesce emits only down to `UsageReaderShared.pollEmitThrottle`); the halves play when the pose changes; Reduce Motion, an open menu, or the pose the app launches in snap instead. There is no idle animation and no gesture on the cost axis: motion reports arrival, not spend. The frames are derived art — regenerating them needs the motion geometry that lives outside this repo, so a re-traced silhouette means re-rendering all 24.
