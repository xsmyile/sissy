...
| `ForgeConnections.swift`        | `ForgeConnection` (kind plus host, port and base path — the id, so one host serving two forges is two rows; `parse` refuses a `user@`, a query, a fragment, any scheme but `https` and a bad port before anything is filed), `ForgeConnectionIndex` (`forge-connections.json`, holding no secret so a lapsed grant still lists what is connected; a file that does not decode is set aside, one whose read fails is left in place, and neither is written over), `ForgeConnector` (probe the token, then file it: a refused token is never saved, and nothing is sent while the index will not read), `ForgeTokenReconciler` (one read of the index against the stored tokens per Settings refresh, and the only way an orphaned token is removed) and `ForgeTokenStore` (`com.radonforge.sissy.forge-token`, whose `storedConnections` Settings reconciles against the index to list tokens no connection names). No enabled flag: a connection is the switch, and removing it is the off |
