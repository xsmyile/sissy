...
`providers[].resets` is what the vendor lets that account spend to clear its
windows early: a count, the count the vendor would apply now, and the soonest
reset's name and lapse. Only Codex publishes one, and only its live reader:
the rollouts carry no count, so `CodexSignals.merge` takes the reader's
whatever the stamps say. Nil is an account nobody knows about, never a zero.
...
| `ClaudeUnsupportedHomes.swift` | Lists the service names of Claude Code's keychain items, attributes only, and reports the scoped ones whose hash is no home Sissy reads: a `claude` signed in under a `CLAUDE_CONFIG_DIR` Sissy does not follow, which Settings names rather than leaving the account missing |
| `AccountUnlink.swift`          | Unlinking one linked account, claude.ai or Codex: the credential first and its recorded name only once that is gone, with either refusal handed back for Settings to show rather than swallowed |
...
| `CodexUsagePayload.swift`       | The one parser for `wham/usage`: the windows in seconds rather than minutes, the plan, the account, the credits through `CodexAdapter.credits`, which is the same block the rollout carries, and the reset count through `CodexResetCredits.summary`. A window whose length is not whole minutes is dropped rather than rounded into a period the vendor never named |
| `CodexResetCredits.swift`       | A Codex account's resets: the count off the usage reply, the list that dates them (`wham/rate-limit-reset-credits`), and the spend (`/consume`), which carries a `redeem_request_id` the vendor redeems once. `CodexResetOutcome` is how a press ended, `already_redeemed` read as the reset it is |
| `CodexUsageSource.swift`        | Polls `chatgpt.com/backend-api/wham/usage` with one account's credential; same 5-min refresh and 429 backoff as the Claude readers. One reader per credential, the CLI's own built with no account of its own. An expired token keeps the previous reading rather than blanking it — something else renews it — and a 401 says so on the row while leaving the windows, which the turns are still writing. Also the one place a reset is spent, since it holds the credential: the list is read only for an account holding one, and an unanswered spend keeps its request id for every press until OpenAI answers it |
...
| `AgentHookInstaller.swift`      | Registers Sissy's `SessionStart` entry with `~/.claude/settings.json` and `~/.codex/hooks.json`, and takes it back out. Off unless the user asks. The path in the command is quoted for `sh` and the result passes `sh -n` before it is written; the home it is built from is `getpwuid`'s, not `NSHomeDirectory()`'s, which follows `CFFIXED_USER_HOME`. A file that will not parse is left untouched, a symlinked target keeps its link, the file's own mode is preserved, the write is abandoned if the file moved between the read and the rename, and a file Sissy itself created is deleted rather than left as `{}` when switching off empties it |
