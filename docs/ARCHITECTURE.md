...
| `ForgeConnections.swift`        | `ForgeConnection` (kind plus host, port and base path — the id, so one host serving two forges is two rows; `parse` refuses a `user@`, a query, a fragment, a wrong scheme and a bad port before anything is filed), `ForgeConnectionIndex` (`forge-connections.json`, holding no secret so a lapsed grant still lists what is connected; a file that will not read is set aside and never written over), `ForgeConnector` (probe the token, then file it: a refused token is never saved) and `ForgeTokenStore` (`com.radonforge.sissy.forge-token`, whose `storedConnections` Settings reconciles against the index to list tokens no connection names). No enabled flag: a connection is the switch, and removing it is the off |
