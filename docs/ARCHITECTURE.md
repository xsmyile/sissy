...
| `ProjectResolver.swift`         | Which project a working directory belongs to: up to the first `.git`, and through a worktree's `gitdir:` pointer to the checkout it was cut from, so one repository is one row wherever the work ran. Cached per directory; a directory that no longer exists is its own project |
